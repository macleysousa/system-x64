class DateTime {
    ...
    AddYears(value = 0) {
        var result = this.DateTimeUtc;
        this.setDate(new Date(result.setFullYear(this.DateTimeUtc.getFullYear() + value)));
        return new DateTime(this.DateTimeUtc);
    }

    AddMonths(value = 0) {
        var result = this.DateTimeUtc;
        this.setDate(new Date(result.setMonth(this.DateTimeUtc.getMonth() + value)));
        return new DateTime(this.DateTimeUtc);
    }

    AddDays(value = 0) {
        var result = this.DateTimeUtc;
        this.setDate(new Date(result.setDate(this.DateTimeUtc.getDate() + value)));
        return new DateTime(this.DateTimeUtc);
    }

    AddHours(value = 0) {
        var result = this.DateTimeUtc;
        this.setDate(new Date(result.setHours(this.DateTimeUtc.getHours() + value)));
        return new DateTime(this.DateTimeUtc);
    }

    AddMinutes(value = 0) {
        var result = this.DateTimeUtc;
        this.setDate(new Date(result.setMinutes(this.DateTimeUtc.getMinutes() + value)));
        return new DateTime(this.DateTimeUtc);
    }

    AddSeconds(value = 0) {
        var result = this.DateTimeUtc;
        this.setDate(new Date(result.setSeconds(this.DateTimeUtc.getSeconds() + value)));
        return new DateTime(this.DateTimeUtc);
    }

    AddMilliseconds(value = 0) {
        var result = this.DateTimeUtc;
        this.setDate(new Date(result.setMilliseconds(this.DateTimeUtc.getMilliseconds() + value)));
        return new DateTime(this.DateTimeUtc);
    }
